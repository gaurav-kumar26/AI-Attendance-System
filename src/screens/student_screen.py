...
def student_screen():
    ...
    login_key = "student_face_login"
    processed_key = f"{login_key}_processed"
    register_key = f"{login_key}_register"

    in_register_mode = st.session_state.get(register_key, False)
    photo_source = get_stored_face_photo(login_key)

    if not in_register_mode:
        st.space()
        photo_source = face_photo_input(
        ...
        with st.spinner('AI is scanning..'):
            ...
            if num_faces == 0:
                ...
                if detected:
                    ...
                    in_register_mode = True

    if st.session_state.get(register_key, False) and photo_source:
        st.info('Face not recognized! Register as a new student below.')
        face_photo_input("", key=login_key, show_camera=False)
        ...
        with st.container(border=True):
            ...
            try:
                ...
                st.error('Audio recording is not available in this browser.')

            if st.button('Create Account', type='primary', key='student_create_account'):
                if new_name:
